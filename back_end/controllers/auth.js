...
const login = async (req, res) => {
  try {
    ...
    if (!errors.isEmpty()) {
      return res.status(400).json({ success: false, message: 'An error occurred\n while logging in', errors: errors.array() });
    }
    ...
    if (user.role === 'HealthcareProvider' && user.status === 'Pending') {
      ...
      return res.status(201).json({ success: false, message: 'Your demand is still under verification.\n Please wait for admin approval .' });
    }
    ...
    await db.User.findOne({
      ...
      if (user) {
        bcrypt.genSalt(SALT, (err, salt) => {
          if (err) return res.status(400).json({ message: "There was an error", error: err });
          else {
            bcrypt.hash(password, salt, async (err, hash) => {
              if (err) return res.status(400)({ message: "There was an error", error: err });
              else {
                user.password = hash;
